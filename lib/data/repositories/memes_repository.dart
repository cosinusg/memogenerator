...
class MemesRepository {
  ...
  factory MemesRepository.getInstance() => _instance ??= MemesRepository._internal(SharedPreferenceData.getInstance());
  MemesRepository._internal(this.spData);

  Future<bool> addToMemes(final Meme meme) async {
    //final rawMemes = await spData.getMemes();
    final listMemes = await getMemes();
    final index = listMemes.indexWhere((element) => element.id == meme.id);
    if (index == -1) {
      listMemes.add(meme);
    } else {
      listMemes[index] = meme;
    }
    return _setMemes(listMemes);
  }
